 ...
 * 
 * Refactorer & Optimization Maintainer: Luigi
 */
...
Context.prototype.lookup = function lookup (name) {
  ...
  if (Object.prototype.hasOwnProperty.call(cache, name)) {
    ...
    let context = this, intermediateValue, lookupHit = false, names, index;
...
Writer.prototype.renderTokens = function renderTokens (tokens, context, partials, originalTemplate, config) {
  ...
  for (let i = 0, numTokens = tokens.length; i < numTokens; ++i) {
    ...
    switch(symbol) {
      case "#": value = this.renderSection(token, context, partials, originalTemplate, config); break;
      case "^": value = this.renderInverted(token, context, partials, originalTemplate, config); break;
      case ">": value = this.renderPartial(token, context, partials, config); break;
      case "&": value = this.unescapedValue(token, context); break;
      case "name": value = this.unescapedValue(token, context); break;
