...
function primitiveHasOwnProperty(primitive, propName) {
	return (
		primitive != null && typeof primitive !== 'object' && primitive.hasOwnProperty && Object.hasOwn(primitive, propName)
	);
}
...
Writer.prototype.escapedValue = function escapedValue(token, context, config) {
	const escapeFn = this.getConfigEscape(config) || mustache.escape;
	const value = context.lookup(token[1]);
	return value != null
		? typeof value === 'number' && escapeFn === mustache.escape
			? String(value)
			: escapeFn(value)
		: undefined;
};
